              ...
              >
                <Box sx={{ mb: 4 }}>
                  <Box sx={{ 
                    ...
                    <Box sx={{ fontSize: 14, mt: 1, opacity: 0.9 }}>
                      예상 준비시간: 5-10분
                    </Box>
                  </Box>
                  ...
                  {contactInfo?.type === 'email' && (
                    <Box sx={{ mb: 2, p: 2, backgroundColor: 'info.light', borderRadius: 1, color: 'white' }}>
                      📧 {contactInfo.email}로 주문 접수 확인 메일을 보내드렸습니다.
                      <br />준비 완료시 다시 알려드리겠습니다.
                    </Box>
                  )}

                  {contactInfo?.type === 'sms' && (
                    <Box sx={{ mb: 2, p: 2, backgroundColor: 'info.light', borderRadius: 1, color: 'white' }}>
                      📱 {contactInfo.phone}로 주문 접수 확인 SMS를 보내드렸습니다.
                      <br />준비 완료시 다시 알려드리겠습니다.
                    </Box>
                  )}

                  {contactInfo?.type === 'none' && (
                    <Box sx={{ mb: 2, p: 2, backgroundColor: 'warning.light', borderRadius: 1, color: 'white' }}>
                      📢 주문번호를 기억해 주세요!
                      <br />준비 완료시 매장에서 호출해 드립니다.
                    </Box>
                  )}

                  <Box sx={{ fontSize: 16, color: 'text.secondary', mb: 3, textAlign: 'center' }}>
                    💡 주문번호 또는 QR코드로 언제든지 주문 상태를 확인하실 수 있습니다.
                  </Box>
                </Box>
              </motion.div>
