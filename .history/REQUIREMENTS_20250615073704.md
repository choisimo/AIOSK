...
**현재 문제점:**
- `order.routes.js`에 관리자용과 공개용 API가 혼재
- `server.js`에서 존재하지 않는 파일 참조
...
graph TD
    ...
    A1[order.routes.js<br/>혼재된 라우트] --> A
    
    B --> B1[routes/admin/orders.routes.js<br/>관리자용 API]
    B --> B2[routes/public/orders.routes.js<br/>공개용 API]
    B --> B3[적절한 server.js 연결]
...
**구체적 작업:**
1. **`routes/admin/orders.routes.js`** 파일 신규 생성
2. 기존 관리자용 라우트 이동 (목록 조회, 상태 변경)
3. **`routes/public/orders.routes.js`** 파일 신규 생성
4. 공개 주문 생성 API 구현
5. `server.js`에서 올바른 경로로 라우터 연결
...
| 구분 | 경로 패턴 | 인증 요구 | 예시 |
|------|-----------|----------|------|
| **관리자용** | `/api/admin/*` | ✅ JWT 필요 | `/api/admin/orders` |
| **공개용** | `/api/public/*` | 🚫 인증 불필요 | `/api/public/menus` |

### 🛡️ 3. 보안 강화 방안

**Admin 회원가입 보안:**
- 현재: 회원가입 라우트 주석 처리 (적절함)
- 권장: 최초 관리자 계정은 CLI 스크립트 또는 DB 직접 생성

**CLI 스크립트 예시:**
```bash
npm run create-admin -- --username admin --password securepassword
```
...
gantt
    ...
    section Phase 1
    공개 API 개발     :active, phase1-1, 2025-06-15, 3d
    라우트 구조 개선   :active, phase1-2, 2025-06-15, 2d
    
    section Phase 2
    파일 업로드 시스템 :phase2-1, after phase1-1, 2d
    주문 취소 기능    :phase2-2, after phase1-1, 1d
    
    section Phase 3
    통계 기능        :phase3-1, after phase2-1, 2d
    로깅 시스템      :phase3-2, after phase2-2, 1d
    
    section Phase 4
    에러 처리 개선    :phase4-1, after phase3-1, 1d
    API 문서화       :phase4-2, after phase3-2, 2d
```

1. 🔴 **공개 API 엔드포인트** (키오스크 운영 필수)
2. 🔴 **라우트 구조 개선** (코드 안정성)
3. 🟠 **파일 업로드 시스템** (관리 편의성)

### Phase 2: 확장 기능 (중간)
4. 🟠 **주문 취소 기능** (운영 편의성)
5. 🟡 **통계 및 리포트** (비즈니스 인사이트)
...
구현 과정에서 질문이나 추가 요구사항이 있으시면 언제든지 말씀해 주세요!
...
> **💡 Pro Tip**: 이 문서는 살아있는 문서입니다. 구현 과정에서 발견되는 새로운 요구사항이나 개선점은 지속적으로 업데이트됩니다.
