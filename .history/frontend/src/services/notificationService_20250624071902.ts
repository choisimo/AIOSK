...
export interface NotificationService {
  sendEmail: (to: string, orderNumber: string, items: any[]) => Promise<void>;
  sendSMS: (to: string, orderNumber: string) => Promise<void>;
}

// 이메일 템플릿
export const createEmailTemplate = (orderNumber: string, items: any[], totalPrice: number) => {
  return {
...
export const mockNotificationService: NotificationService = {
  sendEmail: async (to: string, orderNumber: string, items: any[]) => {
