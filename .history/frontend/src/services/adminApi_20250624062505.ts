...
export const adminApi = {
  ...
  statistics: {
    ...
    // 매출 통계
    getSalesStatistics: async (startDate?: string, endDate?: string): Promise<SalesStatistics> => {
      const params = { startDate, endDate };
      const response = await apiClient.get<SalesStatistics>('/api/admin/statistics/sales', { params });
      return handleApiResponse(response);
    },
    ...
    getDailyStatistics: async (startDate?: string, endDate?: string): Promise<DailyStatistics[]> => {
      const params = { startDate, endDate };
      const response = await apiClient.get<DailyStatistics[]>('/api/admin/statistics/daily', { params });
      return handleApiResponse(response);
    },

    // 시간별 통계
    getHourlyStatistics: async (date?: string): Promise<HourlyStatistics[]> => {
      const params = { date };
      const response = await apiClient.get<HourlyStatistics[]>('/api/admin/statistics/hourly', { params });
      return handleApiResponse(response);
    },

    // 카테고리별 통계
    getCategoryStatistics: async (startDate?: string, endDate?: string): Promise<CategoryStatistics[]> => {
      const params = { startDate, endDate };
      const response = await apiClient.get<CategoryStatistics[]>('/api/admin/statistics/category', { params });
      return handleApiResponse(response);
    },
