...
```bash
# 로그인 페이지 접근 테스트
curl -I http://localhost:4000/admin/login
# → HTTP/1.1 200 OK ✅

# 대시보드 접근 테스트 (로그인 후)
curl -b cookies.txt -L http://localhost:4000/admin
# → 완전한 HTML 응답 확인 ✅
```
...
- [ ] 올바른 URL 사용 (`http://localhost:4000/admin`)
- [ ] 브라우저 캐시 완전 삭제
- [ ] 다른 브라우저로 테스트
- [ ] 방화벽/안티바이러스 일시 비활성화
- [ ] 네트워크 프록시 설정 확인
